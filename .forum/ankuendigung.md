...
| **Fox ESS EV Charger** (Modelle A/L/C) | 🧪 | Auch aus einer Forum-Anfrage entstanden. Ganz normales binäres Modbus TCP: Ladestatus, Spannung/Strom je Phase, Leistung, Energie gesamt + aktuelle Sitzung, Geräteinfo, Steuerung bis 32 A. Optionale Phasenumschaltung, nur wirksam mit externer Phasenumschalt-Box |
| **Peblar** (Home/Home Plus/Business) | 🧪 experimentell | Auf Wunsch eines Forum-Testers aufgenommen. Binäres Modbus TCP (Port 502, Firmware 1.6+, Modbus-Server im Web-Interface aktivieren): Ladestatus, Leistung, Energie, Werte je Phase, Steuerung (Ladefreigabe, Stromlimit), Einphasig-Erzwingen bei Geräten mit unabhängigem Relais. Registerkarte gegen Peblars offiziellen Beispielclient und evcc gegengelesen, noch nicht an echter Hardware bestätigt |
