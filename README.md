...
![Modul Version](https://img.shields.io/badge/Modul_Version-0.9.91--beta.1-blue)
...
| **Alfen Eve Single/Double Pro-line, NG9xx** | Sockel-Status, Ladeleistung, angewandtes Stromlimit, optional Spannung/Strom je Phase, Steuerung (Ladefreigabe, Stromlimit), Phasenumschaltung 1/3 (Register 1215) | Holding-Register (FC 0x03), Float32 Big-Endian. Nur Sockel 1 (Basisadresse ohne Offset) wird bedient. |
...
| 🧪 **Peblar Home/Home Plus/Business** (auch ChargeLine) — **experimentell** | Ladestatus (CP A–F), Leistung, Energie gesamt, Leistung/Spannung/Strom je Phase, Seriennummer/Produkt/Firmware, „Begrenzt durch", Kabelsperre; Steuerung: Ladefreigabe (= Modbus-Stromlimit 0), Stromlimit, Einphasig erzwingen (nur mit unabhängigem Relais) | Binäres Modbus TCP, Unit-ID 1 (an echter Hardware bestätigt; 255 laut Beispielclient nicht geprüft), Port 502, Firmware 1.6+, Modbus-Server im Web-Interface aktivieren, Smart-Charging auf „Standard". Messwerte Input-Register (FC 0x04) ab 30000, Steuerung Holding ab 40000. Registerkarte gegen Peblars offiziellen Beispielclient und evcc gegengelesen. **Lesen, Ladefreigabe, Stromlimit und Phasenumschaltung an echter Hardware bestätigt (Forum-Tester, 20.09.2026).** |
| 🧪 **Phoenix Contact CHARX SEC-3xxx** (Ladesteuerung) — **experimentell** | Ladestatus (IEC 61851-1, ASCII), Leistung, Energie gesamt + aktueller Ladevorgang, Spannung/Strom je Phase, Kennung/Software-Version, Freigabe-Art, Fehlercode; Steuerung: Ladefreigabe (x300) und Stromlimit 6–80 A (x301) | Binäres Modbus TCP, Unit-ID 1, Port 502, 16-Bit-Holding-Register (Holding und Input identisch), 32-Bit-Werte MSW zuerst. Ein Registerblock je Ladepunkt (Startadresse = Nummer × 1000, 1000–48000): in ChargerHub „Nummer des Ladepunkts“ einstellen, eine Instanz je Ladepunkt. Im WBM Modbus-Server aktivieren, Port 502 freigeben, für die Steuerung die Freigabe-Art „Modbus“ wählen; Startadressen besser fest vergeben (bei „automatisch“ können sie sich nach einem Neustart ändern). Registerkarte aus dem Handbuch CHARX SEC-XXXX (109999_en_09, Anhang 8.4). **Noch nicht an echter Hardware bestätigt.** Nicht umgesetzt: Watchdog (x306/x307), Phasenumschaltung (nicht dokumentiert). |
...
## Symbox-Gateway (eingebauter RS485-Port)
...
| `manufacturer` | string | Interner Hersteller-Schlüssel (`goe`/`keba`/`alfen`/`heidelberg`/`abl`/`daheimlader`/`foxess`/`peblar`/`charx`) |
