...
| OCPPHub | neu (29.08.2026, noch kein Release) | ems-integration | `OHUB_GetFunctions` **1.0** (geplant, noch nicht veröffentlicht) — Geschwistervertrag zu `CHUB_GetFunctions`, feldgleich + additiv `transport`/`ocppVersion`, eigener `contractVersion`-Zähler unabhängig von ChargerHub. OCPP-1.6J-Central-System für Wallboxen. |
...
| TibberGridRewards | 2.0.0 main / 2.8.5 beta | Store | `TIBBERGR_GetPriceCurve` **1.1**, `TIBBERGR_GetTariffConfig` **1.1**, `TIBBERGR_GetActiveControls` **2.0** (31.08.2026: `deviceId` liefert jetzt Tibbers echte `vehicleId`/`batteryId` als `string` statt hart `0` als `int` — Major-Bruch für jeden Konsumenten, der `deviceId` bisher als `int`/immer-0 behandelt hat; die ID ist NICHT die lokale IPS-Instanz-ID, nur ohne Kreuzreferenz nicht automatisch zuordenbar, aber stabil über mehrere Aufrufe), `TIBBERGR_SetVehicleSetting` (Abfahrtszeit/Mindest-SoC-Präferenz, kein Vertrag mit contractVersion — reverse-engineerte externe API, siehe SUITE.md-Historie) — main ohne Felder → gilt als 1.0 |
